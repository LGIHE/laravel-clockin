...
@php
    ...
    // Filter navigation items based on user role and custom conditions
    $filteredItems = array_filter($navigationItems, function($item) use ($userRole) {
        $hasRole = in_array($userRole, $item['roles'] ?? []);
        $meetsCondition = !isset($item['showCondition']) || $item['showCondition'] === true;
